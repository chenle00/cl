...
<body class="gray-bg">
				...
				<div class="modal-body">
					<form id="form_user" method="post" action="reserveUser.htm">
						...
						<table style="border-collapse:separate; border-spacing:0px 10px;">
							...
							<tr>
								<td>学生生日：</td>
								<td><input placeholder="学生生日" id="sdbirth" name="sdbirth" class="laydate-icon form-control layer-date"/></td>
								<td>&nbsp;&nbsp;</td>
	...
	<div class="modal fade" id="modal_user_import" role="dialog" aria-labelledby="modal_user_import" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-body">
						<form id="personId" action="importStu.htm" method="post" enctype="multipart/form-data">
    ...
    //外部js调用
    laydate({
        elem: '#sdbirth', //目标元素。由于laydate.js封装了一个轻量级的选择器引擎，因此elem还允许你传入class、tag但必须按照这种方式 '#id .class'
        event: 'focus', //响应事件。如果没有传入event，则按照默认的click
        format: 'YYYY-MM-DD'// 日期格式
    });
    			    ...
    			    success:function(res){
    			    	...
    			    	if(res.success){
    		    			...
    		    			res.elist.forEach(function(obj){
    		    				xdata.push(obj.mdname);
    		    				ydata.push({value:obj.num,name:obj.mdname});
    		    			});
    		    			//设置图表  饼状图
    		    			option = {
	    			    		    title: {
	...
	function initEditUser(getSelection){
		...
		$("input[value='"+getSelection.sdsex+"']").prop("checked",true);//性别   
